...
| `MtngTools.Core.UI` | The shared framework-agnostic UI assembly: the DisplaySelection plan vocabulary — a plan for choosing a display, never a display identifier — with the continuous display→window reconciler, and window-geometry resolution — per-axis offset/size unions, labels and composites, resolved by pure formulas over a supplied reference rectangle with overflow detected, never clamped ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.UI/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
