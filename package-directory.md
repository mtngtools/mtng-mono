...
| `MtngTools.AppCore.MTWindows` | The MTWindows family's framework-agnostic heart — today the slot engine: the three-row reconcile on structural equality of the deserialized config, create-then-swap so a failed mount leaves the incumbent running, and cancel-then-dispose-under-timeout unmount that abandons a wedged component without wedging the slot ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.MTWindows/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
