...
| `MtngTools.Core.UI` | The shared framework-agnostic UI assembly: the DisplaySelection plan vocabulary — a plan for choosing a display, never a display identifier — the continuous display→window reconciler, window-geometry resolution (per-axis offset/size unions, labels and composites, resolved by pure formulas over a supplied reference rectangle with overflow detected, never clamped), the slot-hosting contracts every component and head shares (the VM-first component registry, the slot context with its scoped broker connection), and the window capability seams a head satisfies rather than reimplements — chrome as the portable intersection two UI frameworks both honour, click-through as the one axis neither of them can express at all ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.UI/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
