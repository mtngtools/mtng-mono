...
| `MtngTools.Core.TimerManager.PresentationInput` | The adapter that subscribes `presentation.state-changed` and serves it to the Timer manager over its `IPresentationInput` port — the class whose absence meant the timer heard nothing; carries the four `SelfContained` context groups inward, declines the schedule subscription it has no producer for out loud rather than silently, and serves the retained-pointer read a room booting into a running session needs ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.TimerManager.PresentationInput/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
