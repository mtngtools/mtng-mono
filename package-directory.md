...
| `MtngTools.Frame.UI.Native` | The native selector, and the only unit in the .NET tree that branches on OS: one call hands a deployable this machine's whole native tail — display enumeration, window placement, click-through and inertness together — or nothing at all, which means an unsupported deployment rather than a quiet fall back to a UI framework's normalized geometry ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Frame/MtngTools.Frame.UI.Native/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
