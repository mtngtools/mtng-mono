...
| `MtngTools.AppCore.MTWindows` | The screen app family's framework-free heart: windows as whole-config desired state the engine diffs rather than recreates, slots that mount a component or paint their resolved static content, and the head seam both travel through — one door in per window, and every verdict it refuses reported on the wire ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.MTWindows/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
...
| `MtngTools.Core.UI` | The shared framework-agnostic UI assembly: the DisplaySelection plan vocabulary — a plan for choosing a display, never a display identifier — and the continuous display→window reconciler, one window slot per physical display resolved live over the display-source seam ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.UI/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
| `MtngTools.Core.UI.StaticComponents` | What a screen shows when nothing else is on it: a closed union of simple painted components — solid, or an image under a fit and an alignment — whose whole contract is that nothing about them changes and nothing about them can fail, with the file reading kept behind an asset store that swaps replaced artwork in place and degrades rather than blanking ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.UI.StaticComponents/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
