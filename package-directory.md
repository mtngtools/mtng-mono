...
| Package | Description | Repo | Status |
| :--- | :--- | :--- | :--- |
| `MtngTools.App.Backdrop.Avalonia` | The floor a screen machine starts at log-on: a frameless opaque window per covered display, reconciled live as displays come and go, so the desktop is never visible even while MTWindows is crashed or restarting. One Avalonia head across all three OS — the suffix is load-bearing, so a later WPF head is a new leaf rather than a rename of the shipping deployable. Never exits over configuration: each source is loaded on its own, so an unparseable file costs that file and the floor still paints ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.Backdrop/apps/MtngTools.App.Backdrop.Avalonia/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
| `MtngTools.AppCore.Backdrop` | The backdrop family's deliberately thin AppCore — backdrop-only logic: the hide-cap state machine (receiver-side clamp to the local cap, a deadline that only ever moves earlier, session-scoped so restarting always paints), the display-coverage configuration port with its shipped default, and the floor itself — one window per covered display, held against the live display set through a framework-free head seam ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.Backdrop/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
