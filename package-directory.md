...
| `MtngTools.AppCore.MTWindows` | The MTWindows family's framework-agnostic heart: the slot engine (three-row reconcile, create-then-swap, unmount under a timeout), the host — `ScreenApp` with its fatal startup self-check, per-window `WindowHost` binding `window.set-slot-content` and `window.close` over the backbone — and window configuration end to end: `window.set-window` carrying the closed `WindowConfig` (validated onto `window.config-rejected`, kind defaults applied), diff-applied so a live window is reconfigured property by property and never recreated ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.MTWindows/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
...
| `MtngTools.Core.Wire` | Mechanical C# mirror of the TS/Zod wire contract, generated from `mtng-wire-schemas`; carries the timer and backdrop message sets and the window domain's dual slices — the slot-content trio and the configuration surface's simple messages (hide/show, apply-state/clear, config-rejected) ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.Wire/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
