...
| Package | Description | Repo | Status |
| :--- | :--- | :--- | :--- |
| `MtngTools.App.MTWindows.Avalonia` | A room's screen machine: the deployable that opens the screens and keeps them open, publishing per flavour for a Windows or a macOS box. It boots from a hand-authored seed alone — every window, its slot contents and the room's branding in one file — so a room comes up with no controller and no broker reachable, and nothing configuration- or packaging-shaped ever ends the process ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.MTWindows/apps/MtngTools.App.MTWindows.Avalonia/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
...
| `MtngTools.AppCore.MTWindows` | The MTWindows family's framework-agnostic heart: the slot engine (three-row reconcile, create-then-swap, unmount under a timeout) and the walking-skeleton host — `ScreenApp` with its fatal startup self-check, per-window `WindowHost` binding `window.set-slot-content` and `window.close` over the backbone ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.MTWindows/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
| `MtngTools.AppCore.MTWindows.Avalonia` | The Avalonia head and the composition root in one unit: the OS windows the engine's seam is realised through, plus the single entry point a deployable calls to wire and climb its way to a painted room. It is handed the machine's native tail rather than choosing one, and opens no windows at all on a machine that has none ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/AppCore/MtngTools.AppCore.MTWindows/README.md#family-assemblies)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
...
| `MtngTools.Core.UI` | The shared framework-agnostic UI assembly: the DisplaySelection plan vocabulary — a plan for choosing a display, never a display identifier — the continuous display→window reconciler, window-geometry resolution (per-axis offset/size unions, labels and composites, resolved by pure formulas over a supplied reference rectangle with overflow detected, never clamped), and the slot-hosting contracts every component and head shares (the VM-first component registry, the slot context with its scoped broker connection) ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.UI/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
| `MtngTools.Core.Wire` | Mechanical C# mirror of the TS/Zod wire contract, generated from `mtng-wire-schemas`; carries the timer and backdrop message sets and the window slot-content trio ([spec](https://github.com/mtngtools/mtng-dotnet-mono/blob/main/src/spec/Core/MtngTools.Core.Wire/README.md)). | [mtng-dotnet-mono](https://github.com/mtngtools/mtng-dotnet-mono) | alpha |
